// prettier-ignore
const CRC8_LOOKUP_TABLE = new Int32Array([
  ,
])

function crc8(data: Uint8Array): number {
  return data.reduce((crc, current) => CRC8_LOOKUP_TABLE[(crc ^ current) & 0xff], 0);
}

function checksum(num: string): string {
  return crc8(Buffer.from(num, "hex")).toString(16).padStart(2, "0");
}

export function toAssetLabel(num: number): string {
  if (num <= 0 || num > 65535) {
    throw new Error(`Label ${num} out of range: min label 1 - max label 65535.`);
  }
  const numHex = num.toString(16).padStart(4, "0");
  return "0" + numHex + checksum(numHex) + "0";
}

export function getRefereneAssetName(assetName: string) {
  return `${ASSET_LABELS.REFERENCE}${assetName.slice(8)}`;
}

export function getReferenceSubject({ policyId, assetName }: { policyId: string; assetName: string }) {
  if ([ASSET_LABELS.FT, ASSET_LABELS.NFT, ASSET_LABELS.REFERENCE].includes(assetName.slice(0, 8))) {
    return `${policyId}${getRefereneAssetName(assetName)}`;
  } else {
    return null;
  }
}

export function isReferenceAssetName({ assetName }: { assetName: string }) {
  return assetName.startsWith(ASSET_LABELS.REFERENCE);
}

export const ASSET_LABELS = {
  REFERENCE: toAssetLabel(100),
  NFT: toAssetLabel(222),
  FT: toAssetLabel(333),
};
